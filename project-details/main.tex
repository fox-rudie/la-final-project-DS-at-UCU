...
$$ \exp(x \cdot y / \sigma^2) = \exp(||x||^2 / 2\sigma^2 + ||y||^2 / 2\sigma^2) \exp(-||x-y||^2 / 2\sigma^2 ) \approx $$
$$ \approx \exp(||x||^2 / 2\sigma^2 + ||y||^2 / 2\sigma^2) \phi(x) \cdot \phi(y) $$
...
$$ \approx \sum_{i} \frac{\phi(q)^T \phi(k_i) v^T_i}{\sum{j} \phi(q) \cdot \phi(k_j)} = 
\frac{ \phi(q)^T \sum_{i}\phi(k_i) \otimes v_i }{ \phi(q) \cdot \sum_{j} \phi(k) } $$

where $\otimes$ denotes the outer product between vectors, $\sigma^2$ corresponds to the temperature.

The advantages of the proposed algorithm are:
\begin{itemize}
RFA has a better memory efficiency than softmax attention with softmax normalization (linear vs. quadratic) i.e.  $ \phi(q) $, $ \sum_{i} \phi(k_i) \otimes v_i $ and $ \sum_{i} \phi(k_i) $  requires space of O(4D + 2Dd). From the opposite side, soft dot-product attention requires O(Nd).
\end{itemize}
...
According to the research \cite{nystrom_attention_paper}, softmax matrix S could be written with the use of the Nystrom method as follows:
$$
